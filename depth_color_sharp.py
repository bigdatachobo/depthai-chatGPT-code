...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        depth_frame_colored = cv2.applyColorMap(depth_frame_colored, cv2.COLORMAP_VIRIDIS)  # 수정된 부분

        # 수정된 부분: min_depth 밑과 max_depth 밖의 값을 흰색으로 표시하는 마스크 생성
        too_close_mask = depth_frame_data < min_depth
        too_far_mask = depth_frame_data > max_depth
        combined_mask = np.logical_or(too_close_mask, too_far_mask)

        # 마스크를 깊이 맵에 적용
        depth_frame_colored[combined_mask] = [255, 255, 255]
